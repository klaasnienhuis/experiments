  ...
  var AppView = Backbone.View.extend({
    ...
    renderSequence(recordedData) {
      ...
      var width = Math.floor(
        parseInt(this.$el.find('input[name="width"]').val())
      );
      var height = Math.floor(
        parseInt(this.$el.find('input[name="height"]').val())
      );
      ...
      var overlayOptions = this.$el.find('select[name="overlay"]').val();
    ...
    getFilename: function(extension) {
      if (this.modelInfo !== null) {
        return (
          this.modelInfo.name.replace(/[^a-z0-9]/gi, "_").toLowerCase() +
          extension
        );
      } else {
